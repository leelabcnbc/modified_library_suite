function [im,snrP,imS] = textureSynthesisNew(params, im0, Niter, cmask, imask, scaleMask)
...
if nargin < 6 || isempty(scaleMask)
    scaleMask = ones(1,Nsc);
end

assert(numel(scaleMask) == Nsc); % size of scaleMask matches Nsc.
...
if all(cmask(:))
    ...
    snrP = NaN; % so that we don't have the problem that some snr's are not there.
end
