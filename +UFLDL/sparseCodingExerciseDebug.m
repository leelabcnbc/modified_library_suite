...
lambda = lambda*2000; % so I can work on normalized epsilon.
...
% numFeatures = 5;
% patches = patches(:, 1:5); % YIMENG: this is D x N, where D is # of pixels in image, and N is # of images.
% numPatches = 5;
...
options.Method = 'cg';  % cg seems better than lbfgs.
options.display = 'off';
options.verbose = 0;
rng(0,'twister');
% Initialize matrices
...
for iteration = 1:200                      
    ...
    options.maxIter = 20;
    [featureMatrix, cost] = minFunc( @(x) UFLDL.sparseCodingFeatureCost(weightMatrix, x, visibleSize, numFeatures, batchPatches, gamma, lambda, epsilon, groupMatrix), ...
