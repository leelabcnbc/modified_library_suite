...
restart=1;
% add deterministic thing that can be done on GPU.
cpu_stream = RandStream('CombRecursive','Seed',0,'NormalTransform','Inversion');
RandStream.setGlobalStream(cpu_stream);
rbm;
hidrecbiases=hidbiases; 
if ~exist('mnistdeepauto_test_vh_gpu.mat','file')
    save mnistdeepauto_test_vh_gpu vishid hidrecbiases visbiases;
else
    oldMat = load('mnistdeepauto_test_vh_gpu.mat');
